...
const PROD_KEY = "alsyed_products";
const STORE_KEY = "alsyed_store";
...
const DEFAULT_PRODUCTS = [
  ...
  { id: 13, category: "Mazda T3500", name: "T3500 Parts — WhatsApp karein", original: 0,     oem: 0,    stock: true },
  { id: 14, category: "Motorcycle",  name: "Honda 125 Brake Shoes",         original: 350,   oem: 220,  stock: true },
  { id: 15, category: "Motorcycle",  name: "Yamaha YBR Chain Sprocket Kit", original: 1800,  oem: 1200, stock: true },
  { id: 16, category: "Engine Oil",  name: "Mobil 1 5W-30 (4L)",           original: 4200,  oem: 0,    stock: true },
  { id: 17, category: "Engine Oil",  name: "Shell Helix HX7 (4L)",         original: 2800,  oem: 0,    stock: true },
  { id: 18, category: "Engine Oil",  name: "Total Quartz 5W-40 (4L)",      original: 3100,  oem: 0,    stock: true },
];
...
• Mazda T3500: "Ji hamare paas hain! Detail ke liye WhatsApp karein: ${store.whatsapp_display}"
• Order/contact: WhatsApp ${store.whatsapp_display} pe refer karo
• Sirf auto parts topic pe raho, short jawab do`;
};

const CATS = ["Toyota","Honda","Suzuki","Kia/Hyundai","Mazda T3500","Motorcycle","Engine Oil","Other"];
...
function Admin({ products, setProducts, store, setStore, onClose }) {
  ...
  const addP = () => {
    ...
    const updated = [...products,{id:Date.now(),category:newP.category,name:newP.name.trim(),
      original:Number(newP.original)||0,oem:Number(newP.oem)||0,stock:newP.stock}];
    setProducts(updated); saveLS(PROD_KEY,updated);
    setNewP({category:"Mazda T3500",name:"",original:"",oem:"",stock:true}); setShowAdd(false);
  };
  const updP = (id,f,v)=>{const u=products.map(p=>p.id===id?{...p,[f]:v}:p);setProducts(u);saveLS(PROD_KEY,u);};
  const delP = (id)=>{const u=products.filter(p=>p.id!==id);setProducts(u);saveLS(PROD_KEY,u);};
  const saveStore = ()=>{setStore(sf);saveLS(STORE_KEY,sf);setSaved(true);setTimeout(()=>setSaved(false),2000);};
    ...
    <div style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.93)",
      ...
      <div style={{width:"100%",maxWidth:480,background:"#0f0f18",borderRadius:"20px 20px 0 0",
        ...
        <div style={{background:"linear-gradient(135deg,#c1121f,#8b0000)",padding:"14px 18px",
          ...
          <div>
            ...
            <div style={{color:"rgba(255,255,255,0.7)",fontSize:11,marginTop:1}}>
              {products.filter(p=>p.stock).length} products in stock
            </div>
          </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"14px 16px 24px"}}>
          {tab==="products"&&(
            <>
              <div style={{background:"#1a0f00",border:"1px dashed #f4a261",borderRadius:10,
                padding:"10px 14px",marginBottom:12,fontSize:12,color:"#f4a261"}}>
                🚛 <b>Mazda T3500 parts add karne ke liye</b> — neeche button dabao!
              </div>
              {!showAdd?(
                ...
                <div style={{background:"#1a0a0a",borderRadius:12,padding:14,border:"1px solid #c1121f40",marginBottom:14}}>
                  ...
                  <div style={{display:"grid",gap:8}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                      ...
                      <input placeholder="OEM Price (0=N/A)" type="number" value={newP.oem} onChange={e=>setNewP(p=>({...p,oem:e.target.value}))} style={IS}/>
                    </div>
          ...
          {tab==="store"&&(
            <div style={{display:"grid",gap:12}}>
              {[["Dukaan Naam","name"],["Owner","owner"],["WhatsApp (923XXXXXXXXX)","whatsapp"],
...
export default function App() {
  const [products, setProducts] = useState(()=>loadLS(PROD_KEY,DEFAULT_PRODUCTS));
    ...
    <div style={{minHeight:"100vh",maxHeight:"100vh",
      ...
      {iosGuide&&(
        <div style={{position:"fixed",inset:0,zIndex:300,background:"rgba(0,0,0,0.92)",
          display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
          <div style={{width:"100%",maxWidth:480,background:"#0f0f18",borderRadius:"20px 20px 0 0",padding:24,border:"1px solid #c1121f40"}}>
            <div style={{color:"#fff",fontWeight:900,fontSize:17,marginBottom:16}}>📱 iPhone Pe Install Karein</div>
            {[["1️⃣","Neeche Share button dabao","(□ arrow waala)"],
              ["2️⃣","'Add to Home Screen' dabao","scroll karke dhundhein"],
              ["3️⃣","'Add' dabao","App install ho jayegi! ✅"]].map(([n,s,sub])=>(
              <div key={s} style={{display:"flex",gap:12,background:"#1a1a2e",borderRadius:10,padding:"11px 14px",marginBottom:8}}>
                ...
                <div>
                  <div style={{color:"#f0f0f0",fontSize:14,fontWeight:600}}>{s}</div>
                  <div style={{color:"#666",fontSize:12,marginTop:2}}>{sub}</div>
                </div>
              </div>
            ))}
            <button onClick={()=>setIosGuide(false)} style={{width:"100%",marginTop:12,padding:"12px",
              background:"#c1121f",border:"none",borderRadius:10,color:"#fff",fontWeight:800,
              fontSize:14,cursor:"pointer",fontFamily:"inherit"}}>Samajh Gaya ✓</button>
          </div>
        </div>
      )}

      {/* HEADER */}
      <div style={{background:"linear-gradient(135deg,#c1121f 0%,#7f0000 100%)",
        padding:"11px 14px",display:"flex",alignItems:"center",gap:11,
        boxShadow:"0 4px 20px rgba(193,18,31,0.4)",flexShrink:0}}>
        <div style={{width:44,height:44,borderRadius:13,background:"rgba(255,255,255,0.15)",
          display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,
          border:"2px solid rgba(255,255,255,0.25)",flexShrink:0}}>🔧</div>
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:5,marginTop:1}}>
            ...
            <span style={{color:"rgba(255,255,255,0.75)",fontSize:11}}>
              Saim AI • {products.filter(p=>p.stock).length} parts ready
            </span>
          </div>
        </div>
        <div style={{display:"flex",gap:7}}>
          {(installPrompt||isIOS)&&(
            <button onClick={install} title="App Install Karein" style={{background:"rgba(255,255,255,0.15)",
              border:"1px solid rgba(255,255,255,0.3)",borderRadius:9,padding:"7px 9px",cursor:"pointer",fontSize:15}}>📲</button>
          )}
          <button onClick={()=>openWA()} style={{background:"#25D366",border:"none",borderRadius:9,
            padding:"7px 9px",cursor:"pointer",fontSize:16,boxShadow:"0 2px 8px rgba(37,211,102,0.4)"}}>📱</button>
          <button onClick={()=>setShowAdmin(true)} style={{background:"rgba(255,255,255,0.15)",
            border:"1px solid rgba(255,255,255,0.25)",borderRadius:9,padding:"7px 9px",cursor:"pointer",fontSize:15}}>⚙️</button>
        </div>
      </div>

      {/* INFO BAR */}
      <div style={{background:"#110505",borderBottom:"1px solid #1f0a0a",
        padding:"6px 14px",display:"flex",gap:14,overflowX:"auto",flexShrink:0}}>
        {[["🕐","9am–9pm"],["📍","Karachi"],["🚚","Delivery"],["🚛","Mazda T3500"],["⭐","Warranty"]].map(([i,t])=>(
          <div key={t} style={{display:"flex",alignItems:"center",gap:3,color:"#555",fontSize:11,whiteSpace:"nowrap",flexShrink:0}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"13px 11px",display:"flex",flexDirection:"column",gap:10}}>
        {messages.map((msg,i)=>(
          <div key={i} style={{display:"flex",justifyContent:msg.role==="user"?"flex-end":"flex-start",
            alignItems:"flex-end",gap:7,animation:"si 0.25s ease"}}>
            {msg.role==="assistant"&&(
            ...
            <div style={{maxWidth:"78%",padding:"10px 13px",
              ...
              background:msg.role==="user"?"linear-gradient(135deg,#c1121f,#7f0000)":"#1c1c2e",
              ...
              {msg.role==="assistant"&&(msg.text.toLowerCase().includes("whatsapp")||msg.text.toLowerCase().includes("order"))&&(
                <button onClick={()=>openWA(`Chatbot se: ${msg.text.substring(0,60)}`)} style={{
                  display:"block",marginTop:8,background:"#25D366",border:"none",borderRadius:8,
                  padding:"7px 12px",color:"#fff",cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:"inherit"}}>
        ...
        {loading&&(
          <div style={{display:"flex",alignItems:"flex-end",gap:7}}>
            <div style={{width:30,height:30,borderRadius:9,background:"linear-gradient(135deg,#c1121f,#7f0000)",
              display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>🔧</div>
            <div style={{background:"#1c1c2e",border:"1px solid #2a2a40",borderRadius:"16px 16px 16px 4px",padding:"11px 15px"}}>
              <Dots/>
            </div>
          </div>
        )}
        {showQuick&&(
          <div style={{marginTop:4}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:6,justifyContent:"center"}}>
              {quickQs.map(q=>(
                <button key={q} onClick={()=>send(q)} style={{background:"transparent",
                  border:"1px solid #3a1010",borderRadius:20,padding:"7px 13px",
                  color:"#c1121f",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>{q}</button>
              ))}
            </div>
          </div>
        )}
      ...
      <div style={{padding:"9px 11px 14px",background:"#08080f",borderTop:"1px solid #1a1a2e",flexShrink:0}}>
        <button onClick={()=>openWA()} style={{width:"100%",padding:"9px",marginBottom:8,
          background:"linear-gradient(135deg,#128C7E,#075E54)",border:"none",borderRadius:10,
          color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit",
          display:"flex",alignItems:"center",justifyContent:"center",gap:7,
          boxShadow:"0 2px 10px rgba(18,140,126,0.3)"}}>
          <span style={{fontSize:16}}>📱</span>
          Direct Order — {store.whatsapp_display} (Syed Imran Shah)
        </button>
        <div style={{display:"flex",gap:7,background:"#1c1c2e",borderRadius:13,
          padding:"7px 7px 7px 13px",border:"1px solid #2a2a40"}}>
          <input ref={inputRef} value={input} onChange={e=>setInput(e.target.value)}
            onKeyDown={e=>e.key==="Enter"&&!e.shiftKey&&send()}
            placeholder="Part ka naam ya sawaal..." disabled={loading}
            style={{flex:1,background:"transparent",border:"none",color:"#f0f0f0",
              fontSize:14,outline:"none",fontFamily:"inherit",caretColor:"#c1121f"}}/>
          <button onClick={()=>send()} disabled={loading||!input.trim()} style={{
            width:38,height:38,borderRadius:9,border:"none",flexShrink:0,
            background:input.trim()&&!loading?"linear-gradient(135deg,#c1121f,#7f0000)":"#1a1a2e",
            color:"#fff",cursor:input.trim()&&!loading?"pointer":"not-allowed",fontSize:15,transition:"all 0.2s"}}>
            {loading?"⏳":"➤"}
          </button>
        </div>
        <div style={{textAlign:"center",marginTop:6,fontSize:10,color:"#1f1f1f"}}>
          AlSyed Autoparts 🇵🇰 • Powered by Claude AI
        </div>
      </div>
